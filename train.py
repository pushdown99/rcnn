  ...
  for epoch in range (opt.epoch):
    ...
    for ii, (img, bbox_, label_, scale) in tqdm (enumerate (dataloader)):
      ...
      if (ii + 1) % opt.plot_every == 0:
        ...
        gt_img = visdom_bbox(ori_img_, at.tonumpy(bbox_[0]), at.tonumpy(label_[0])) trainer.vis.img('gt_img', gt_img)

        # plot predicti bboxes
        _bboxes, _labels, _scores = trainer.faster_rcnn.predict([ori_img_], visualize=True)
        pred_img = visdom_bbox(ori_img_, at.tonumpy(_bboxes[0]), at.tonumpy(_labels[0]).reshape(-1), at.tonumpy(_scores[0])) trainer.vis.img('pred_img', pred_img)
    ...
    if eval_result['map'] > best_map:
      print (eval_result['map'], best_map)
