...
class Hierarchical_Descriptive_Model(HDN_base):
    ...
    def forward(self, im_data, im_info, gt_objects=None, gt_relationships=None, gt_regions=None, 
        ...
        roi_data_object, roi_data_predicate, roi_data_region, mat_object, mat_phrase, mat_region = \
            self.proposal_target_layer(object_rois, region_rois, gt_objects, gt_relationships, gt_regions, self.n_classes_obj, self.voc_sign, self.training, graph_generation=graph_generation)
        if TIME_IT:
            torch.cuda.synchronize()
        ...
        cls_prob_object = F.softmax(cls_score_object, -1)

        cls_score_predicate = self.score_pred(pooled_phrase_features)
        cls_prob_predicate = F.softmax(cls_score_predicate, -1)
    ...
    def interpret_HDN(self, cls_prob, bbox_pred, rois, cls_prob_predicate, 
        ...
        boxes = rois.data.cpu().numpy()[keep, 1:5] / im_info[0][2].numpy() #hyhwang
        if use_gt_boxes:
            nms = False
            clip = False
            pred_boxes = boxes
    ...
    def build_loss_objectiveness(self, region_objectiveness, targets):
        ...
        loss_objectiveness = F.cross_entropy(region_objectiveness, targets, reduction='sum')
